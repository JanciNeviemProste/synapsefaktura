...
const CLIENTS = [
  { firm: "Novastav s. r. o.", docs: 34, vat: "platiteľ", peppol: "aktívne", ok: true },
  { firm: "Pekáreň Vlčkovci", docs: 18, vat: "platiteľ", peppol: "aktívne", ok: true },
  { firm: "M. Kováč — KOVEX", docs: 11, vat: "neplatiteľ", peppol: "—", ok: true },
  { firm: "Ateliér Sova", docs: 7, vat: "platiteľ", peppol: "čaká na ID", ok: false },
]
...
export default function V4Page() {
  return (
    ...
    >
      {/* Header */}
      <header className="bg-white" style={{ borderBottom: `1px solid ${RULE}` }}>
        <div className="mx-auto flex h-16 max-w-6xl items-center justify-between px-6">
          ...
          <nav className="flex items-center gap-5 text-sm">
            <Link href="/login" className="hover:underline underline-offset-4" style={{ color: GRAY }}>
              Prihlásiť sa
            </Link>
            <Link
              href="/register"
      ...
      <section className="mx-auto grid max-w-6xl items-center gap-12 px-6 py-20 lg:grid-cols-[1fr_1.15fr]">
        <div className="flex flex-col gap-5">
          <p className="text-xs font-semibold tracking-[0.18em] uppercase" style={{ color: BLUE }}>
          ...
          <p className="max-w-md text-base leading-relaxed" style={{ color: GRAY }}>
            Spravujete doklady pre viac firiem? Synapse Faktúra ich drží pod jedným
            účtom — s AI vyťažovaním dokladov, prísnym oddelením dát a e-faktúrou
            2027 pripravenou za všetkých naraz.
          </p>
        ...
        >
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-xs uppercase" style={{ color: GRAY }}>
            ...
            <tbody>
              {CLIENTS.map((c) => (
                <tr key={c.firm} style={{ borderTop: `1px solid ${RULE}` }}>
                  ...
                  <td className="px-2 py-3 text-right tabular-nums">{c.docs}</td>
                  ...
                  <td className="px-5 py-3">
                    <span className="flex items-center gap-1.5">
                      <span
                        ...
                        style={{ backgroundColor: c.ok ? "#12805C" : "#B78103" }}
                      />
      ...
      <section id="cennik" className="mx-auto max-w-6xl px-6 py-16">
        ...
        <p className="mb-8 max-w-xl text-sm leading-relaxed" style={{ color: GRAY }}>
          Pre kancelárie odporúčame Business — viac používateľov, odosielanie
          e-faktúr a API. Každý plán vyskúšate 14 dní zdarma.
        </p>
        <div className="overflow-x-auto rounded-md bg-white" style={{ border: `1px solid ${RULE}` }}>
              ...
              <tr style={{ borderBottom: `1px solid ${RULE}` }}>
                ...
                {PLAN_ORDER.map((t) => (
                  <td key={t} className="px-4 py-3 text-center tabular-nums">
                    {PLANS[t].docsPerMonth === null ? "neobmedzené" : PLANS[t].docsPerMonth}
                  </td>
                ))}
              </tr>
              ...
              <tr>
                ...
                {PLAN_ORDER.map((t) => (
                  <td key={t} className="px-4 py-4 text-center">
                    ...
                    >
                      {t === "free" ? "Začať zadarmo" : `Vyskúšať ${PLANS[t].label}`}
                    </Link>
                  </td>
                ))}
              </tr>
      ...
      <section style={{ backgroundColor: INK }}>
        <div className="mx-auto flex max-w-6xl flex-col items-start justify-between gap-6 px-6 py-16 text-white sm:flex-row sm:items-center">
          <div>
            <h2 className="text-2xl font-bold">Prvého klienta máte v systéme za 10 minút.</h2>
            <p className="mt-1 text-sm opacity-70">
              Vyskúšajte s jednou firmou — ďalšie pridáte, keď to bude dávať zmysel.
            </p>
          </div>
          <Link
            href="/register"
