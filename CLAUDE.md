...
`main`: `typecheck` PASS · `lint` PASS · `test` **387/387** PASS · `build` PASS.
**19 migrácií** sa aplikuje načisto na PostgreSQL 17 (36 tabuliek / 25 enumov /
81 policies / 10 funkcií, 0 tabuliek bez RLS).

Nasadené na `synapsefaktura.vercel.app`. Supabase `oukooqfpxeunhdzndsid` je
`ACTIVE_HEALTHY` — **36 tabuliek, 25 enumov, 81 policies, 9 funkcií,
15 migrácií**; schéma aj evidencia migrácií sedia s repom 1:1. Keep-alive
workflow beží (HTTP 200).
