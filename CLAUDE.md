...
(po merge PR #2).
**PR #10** (fázy 1–6 + opravy z recenzie): `typecheck` PASS · `lint` PASS ·
`test` **356/356** · `build` PASS; všetkých 15 migrácií sa aplikuje načisto
na PostgreSQL 17.

Nasadené na `synapsefaktura.vercel.app`. Supabase `oukooqfpxeunhdzndsid` je
`ACTIVE_HEALTHY` — **36 tabuliek, 23 enumov, 81 policies, 10 migrácií**
(schéma je pred kódom: fázy 3 a 4 z PR #6/#7 sú aplikované, kód ešte nie je
zmergovaný); keep-alive workflow beží (HTTP 200).
